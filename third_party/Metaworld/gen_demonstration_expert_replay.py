...
def load_mw_data(save_dir):
    ...
    return action,info_near,info_grasp,total_sub

# def contact(action, info_near, info_grasp, if_near=False, if_near_grasp=False):
#     # actions = [action[0]]  # 从第一个 action 开始
# 	# print(len(action))
# 	actions = []
# 	if if_near:
...
# 			else:
# 				actions.append(action[i] + action[i + 1])
# 				i += 1
# 			i += 1
...
# def process_all_episodes(actions, info_near, info_grasp, total_sub, if_near=False, if_near_grasp=False):
# 	old_actions_len = len(actions)
# 	new_actions = []
# 	new_total_sub = []
# 	start_idx = 0

# 	for episode_length in total_sub:
# 		action_episode = actions[start_idx:start_idx + episode_length]
# 		info_near_episode = info_near[start_idx:start_idx + episode_length]
# 		print(info_near_episode)
# 		info_grasp_episode = info_grasp[start_idx:start_idx + episode_length]

# 		processed_actions = contact(action_episode, info_near_episode, info_grasp_episode, if_near, if_near_grasp)

# 		new_actions.extend(processed_actions)
# 		new_total_sub.append(len(processed_actions))
# 		print(len(processed_actions))

# 		start_idx += episode_length
# 	speed = old_actions_len/(len(new_actions))
# 	print("speed",speed)
# 	return new_actions, new_total_sub,speed
...
def main(args):
	...
	save_dir = os.path.join(args.root_dir, 'metaworld_'+args.env_name+f'_expert_speed{speed}.zarr')
	seed = 73
	...
	action_data,info_near_data,info_grasp_data,total_sub = load_mw_data(save_dir_old)
	if speed==0:
		if_near = False
		if_near_grasp = False
		...
		while episode_idx < num_episodes:
			...
			while not done:
				...
				if quit(action_idx_episode_idx,total_sub[episode_idx]):
					if if_near:
						...
						if info['near_object']!=info['grasp_success']:
							action = action_data[action_idx]
						else:
							if quit(action_idx_episode_idx+1,total_sub[episode_idx]):
								action = action_data[action_idx]+action_data[action_idx+1]
			...
			if not ep_success or ep_success_times < 5:
				...
				for n, ax in enumerate(axes):
					...
					ax.plot(tstep, np.array(qpos_sub)[1:, n], label=f'target qpos {n}')
				...
				if n_groups == 1:
					...
					axes.plot(tstep, np.array(qpos_gripper_sub)[1:], label=f'target qpos')
